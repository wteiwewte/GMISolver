...
private:
    ...
    std::string basicInformationStr() const
    {
        ...
        oss << "CONSTRAINT MATRIX DIMENSIONS: " << _constraintMatrix.size() << " x " << _constraintMatrix.front().size() << '\n';
...
template<typename T>
std::string LinearProgram<T>::toString() const {
...
template<typename T>
void LinearProgram<T>::convertToStandardForm() {
    ...
    const auto newSlackLabel = [&] (){
        ...
        return (_variableLabelSet.find(firstPattern) == _variableLabelSet.end()) ? firstPattern : firstPattern + Constants::SLACK_SUFFIX;
    };
    ...
    for (int rowIdx = 0; rowIdx < _rowInfos.size(); ++rowIdx)
    {
        ...
        _variableInfos.push_back(VariableInfo{newSlackLabelStr, VariableType::CONTINUOUS, true});
...
template<typename T>
void LinearProgram<T>::makeRightHandSidesNonNegative() {
    for (int rowIdx = 1; rowIdx < _rowInfos.size(); ++rowIdx) {
        if (_rightHandSides[rowIdx] < 0.0)
        {
            for (int variableIdx = 0; variableIdx < _variableInfos.size(); ++variableIdx)
                _constraintMatrix[rowIdx][variableIdx] = -_constraintMatrix[rowIdx][variableIdx];
...
template<typename T>
void LinearProgram<T>::addArtificialVariables() {
    const int variableCountAtTheStart = _variableInfos.size();
    ...
    const auto newArtificialLabel = [&] (const auto varIdx){
        ...
        return (_variableLabelSet.find(firstPattern) == _variableLabelSet.end()) ? firstPattern : firstPattern + Constants::ARTIFICIAL_SUFFIX;
    };
    ...
    for (int rowIdx = 0; rowIdx < _rowInfos.size(); ++rowIdx)
    {
        ...
        _variableInfos.push_back(VariableInfo{newArtificialLabelStr, VariableType::CONTINUOUS, false, true, true});
