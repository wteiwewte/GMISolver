...
public:
        ...
        _rightHandSides(linearProgram._rightHandSides)
    {
        ...
        for (int columnIdx = 1; columnIdx < _basisMatrixInverse.size(); ++columnIdx)
            _basisMatrixInverse[0][columnIdx] = objectiveRow[linearProgram.initialVariableCountInStandardForm() + columnIdx - 1] *
                    _constraintMatrix[columnIdx][linearProgram.initialVariableCountInStandardForm() + columnIdx - 1];

        for (int rowIdx = 1; rowIdx < _basisMatrixInverse.size(); ++rowIdx)
        {
            _basisMatrixInverse[rowIdx].resize(_basisMatrixInverse.size());
            _basisMatrixInverse[rowIdx][rowIdx] = _constraintMatrix[rowIdx][linearProgram.initialVariableCountInStandardForm() + rowIdx - 1];
        }
    ...
    std::string toString() const
    {
        ...
        lpPrinter.printLineBreak();
        lpPrinter.printMatrixWithRHS(_rowToBasisColumnIdxMap, _constraintMatrix, _rightHandSides);
        lpPrinter.printLineBreak();
    ...
    bool primalSimplex()
    {
        ...
        {
//            if (_reducedCosts[varIdx] < 0.0 && _variableInfos[varIdx]._isBasic)
//                spdlog::warn
            std::cout << "VARIABLE IDX " << varIdx << ' '  << _constraintMatrix[0][varIdx] << '\n';
            if(_constraintMatrix[0][varIdx] < 0.0 && !_variableInfos[varIdx]._isBasic)
            {
                ...
                for (int rowIdx = 1; rowIdx < _rowInfos.size(); ++rowIdx)
                    if (_constraintMatrix[rowIdx][*enteringVarIdx] > 0.0)
                        if (!leavingRowIdx.has_value() ||
                            _rightHandSides[rowIdx] * _constraintMatrix[*leavingRowIdx][varIdx] <
                            _rightHandSides[*leavingRowIdx] * _constraintMatrix[rowIdx][varIdx])
                ...
                const auto pivotingTermInverse = 1.0 / _constraintMatrix[*leavingRowIdx][*enteringVarIdx];

                for (int i = 0; i < _rowInfos.size(); ++i)
                {
                    if (i != *leavingRowIdx) {
                        const auto commonCoeff = _constraintMatrix[i][*enteringVarIdx] * pivotingTermInverse;
                        for (int j = 0; j < _variableInfos.size(); ++j)
                            _constraintMatrix[i][j] -=
                                    commonCoeff *
                                    _constraintMatrix[*leavingRowIdx][j];

//                        if (i == 0)
//                            std::cout << "COMMON COEFF " << commonCoeff << " " << _rightHandSides[*leavingRowIdx] << " "
//                             << _rightHandSides[i] << '\n';
                        // ???
                        if (i == 0)
                ...
                std::cout << "LEAVING VARIABLE COLUMN IDX " << _rowToBasisColumnIdxMap[*leavingRowIdx] << '\n';
                _variableInfos[_rowToBasisColumnIdxMap[*leavingRowIdx]]._isBasic = false;
