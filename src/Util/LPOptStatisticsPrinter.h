...
template <typename T> struct LPOptStatisticsPrinter {
  ...
  LPOptStatisticsPrinter(const LPOptStatisticsVec<T> &lpOptStatisticsVec)
    ...
    _totalWidth = LP_NAME_WIDTH + SIMPLEX_ALGO_TYPE_WIDTH +
                  LP_OPT_RESULT_WIDTH + OPT_VALUE_WIDTH +
                  ITERATION_COUNT_WIDTH + REINVERSION_FREQUENCY_WIDTH;
  ...
  void printFirstLine() {
    ...
    _oss << fmt::format("{:^{}}|", "SIMPLEX ALGO TYPE",
                        SIMPLEX_ALGO_TYPE_WIDTH);
  ...
  void printLPOptStatistics(const LPOptStatistics<T> &lpOptStatistics) {
    ...
    _oss << fmt::format("{:^{}}|", lpOptStatistics._simplexAlgorithmType,
                        SIMPLEX_ALGO_TYPE_WIDTH);
    _oss << fmt::format("{:^{}}|",
                        lpOptimizationResultToStr(lpOptStatistics._optResult),
                        LP_OPT_RESULT_WIDTH);
    _oss << fmt::format("{:^{}}|", lpOptStatistics._optimalValue,
                        OPT_VALUE_WIDTH);
