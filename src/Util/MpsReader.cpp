...
std::optional<LinearProgram<T>> MpsReader::read(const std::string &filePath) {
    ...
    while(std::getline(fileStream, readLine))
    {
        ...
        switch(currentSection)
        {
            ...
            case SectionType::ROWS:
            {
                if (lineParts.size() != 2)
                {
                    spdlog::warn("Line {} specifying row should have exactly 2 parts", readLine);
                ...
                const auto [_, inserted] = rowLabelToRowIdxMap.try_emplace(rowLabelStr, linearProgram._rowInfos.size());
                if (!inserted)
            ...
            case SectionType::COLUMNS:
            {
                ...
                {
                    spdlog::warn("Unexpected number of elements in column line {}", readLine);
                    break;
                }
                ...
                if (lineParts.size() == 3 && lineParts[1] == MARKER_KEYWORD)
                {
                    ...
                    if (integerSectionStr == INTEGER_SECTION_START_KEYWORD)
                        ...
                        spdlog::warn("Unrecognized integer section keyword {}", integerSectionStr);
                ...
                if (variableLabelStr.find(Constants::SLACK_SUFFIX) != std::string::npos ||
                        variableLabelStr.find(Constants::ARTIFICIAL_SUFFIX) != std::string::npos)
                {
                    spdlog::warn("Disallowed variable label {}", variableLabelStr);
                    break;
                }

                const auto [variableIt, inserted] = variableLabelToVariableIdxMap.try_emplace(variableLabelStr, linearProgram._variableInfos.size());
                const auto variableIdx = variableIt->second;
                if (inserted)
                {
                    linearProgram._variableInfos.push_back(VariableInfo{variableLabelStr, currentSectionIsInteger ? VariableType::INTEGER : VariableType::CONTINUOUS});
                ...
                const auto updateLpMatrix = [&](const auto& rowLabelStr, const auto& coefficientValueStr){
                    const auto foundRowIt = rowLabelToRowIdxMap.find(rowLabelStr);
                    if (foundRowIt == rowLabelToRowIdxMap.end())
                    {
                        spdlog::warn("Row {} given in column section doesn't correspond to any row", rowLabelStr);
                        return;
                    }

                    linearProgram._constraintMatrix[foundRowIt->second][variableIdx] = convert(coefficientValueStr);
                };
            ...
            case SectionType::RHS:
            {
                ...
                const auto updateLpRhs= [&](const auto& rowLabelStr, const auto& coefficientValueStr){
                    const auto foundRowIt = rowLabelToRowIdxMap.find(rowLabelStr);
                    if (foundRowIt == rowLabelToRowIdxMap.end())
                    {
                        spdlog::warn("Row label {} given in column section doesn't correspond to any row", rowLabelStr);
                        return;
                    }

                    linearProgram._rightHandSides.resize(linearProgram._rowInfos.size());
                    linearProgram._rightHandSides[foundRowIt->second] = convert(coefficientValueStr);
                };
            ...
            case SectionType::BOUNDS:
            {
                if (lineParts.size() != 4)
                {
                    spdlog::warn("Unexpected number of elements in bounds line {}", readLine);
                    break;
                }
                ...
                const auto foundVariableIt = variableLabelToVariableIdxMap.find(variableStr);
                if (foundVariableIt == variableLabelToVariableIdxMap.end())
                {
                    spdlog::warn("Variable label {} given in bounds section doesn't correspond to any variable", variableStr);
                    break;
                }
                ...
                linearProgram._constraintMatrix.emplace_back(linearProgram._variableInfos.size())[variableIdx] = 1;
                switch (*readBoundType)
                {
                    ...
                    case BoundType::LOWER_BOUND:
                    {
                        ...
                        linearProgram._rowInfos.push_back(RowInfo{{}, RowType::GREATER_THAN_OR_EQUAL});
                        break;
                    }
                    case BoundType::UPPER_BOUND:
                    {
                        linearProgram._rightHandSides.push_back(convert(coefficientValueStr));
                        linearProgram._rowInfos.push_back(RowInfo{{}, RowType::LESS_THAN_OR_EQUAL});
                        break;
                    }
    ...
    if (linearProgram._constraintMatrix.empty() || linearProgram._variableInfos.empty())
    {
    ...
    {
        spdlog::warn("Number of constraints doesn't match number of right hand sides");
        return std::nullopt;
    }
...
template std::optional<LinearProgram<double>> MpsReader::read(const std::string&);