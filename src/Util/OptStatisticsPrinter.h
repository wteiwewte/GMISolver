...
struct OptStatisticsPrinter {
  ...
  constexpr static int OPT_COUNT_WIDTH = 12;
  ...
  template <typename T> void print(const LPOptStatistics<T> &lpOptStatistics) {
    ...
    _oss << fmt::format("{:^{}}|",
                        ...
                        OPT_RESULT_WIDTH);
    _oss << fmt::format("{:^{}}|",
                        valueWithPrecision(lpOptStatistics._optimalValue),
                        OPT_VALUE_WIDTH);
  ...
  template <typename T> void print(const IPOptStatistics<T> &ipOptStatistics) {
    ...
    _oss << fmt::format("{:^{}}|",
                        ...
                        OPT_RESULT_WIDTH);
    _oss << fmt::format("{:^{}}|",
                        valueWithPrecision(ipOptStatistics._optimalValue),
                        OPT_VALUE_WIDTH);
    _oss << fmt::format("{:^{}}|", ipOptStatistics.totalIterationCount(),
                        ITERATION_COUNT_WIDTH);
    _oss << fmt::format("{:^{}}|", ipOptStatistics._elapsedTimeSec,
                        ELAPSED_TIME_WIDTH);
  ...
  void print(const LexReoptStatistics<T> &lexReoptStatistics) {
    ...
    _oss << fmt::format(
        ...
        OPT_RESULT_WIDTH);
    _oss << fmt::format("{:^{}}|",
                        valueWithPrecision(lexReoptStatistics._optimalValue),
                        OPT_VALUE_WIDTH);
    _oss << fmt::format("{:^{}}|", lexReoptStatistics.totalIterationCount(),
                        ITERATION_COUNT_WIDTH);
    _oss << fmt::format("{:^{}}|", lexReoptStatistics._elapsedTimeSec,
                        ELAPSED_TIME_WIDTH);
  ...
  void print(const PrimalSimplexOutput<T> &primalSimplexOutput) {
    print(primalSimplexOutput._phaseOneLpOptStats);
    if (primalSimplexOutput._phaseTwoLpOptStats.has_value()) {
      print(primalSimplexOutput._phaseTwoLpOptStats.value());
    }
  }

  template <typename T> std::string valueWithPrecision(const T value) const {
    return fmt::format("{:.{}f}", value, FLOAT_NUMBERS_PRECISION);
  }
